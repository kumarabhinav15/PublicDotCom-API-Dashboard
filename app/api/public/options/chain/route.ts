...
export async function POST(req: Request) {
  ...
  if (env.mock) return NextResponse.json(mock.chain(parsed.data.instrument.symbol, parsed.data.expirationDate));

  const res = await publicFetch(`/userapigateway/marketdata/${encodeURIComponent(accountId)}/option-chain`, {
    method: 'POST',
