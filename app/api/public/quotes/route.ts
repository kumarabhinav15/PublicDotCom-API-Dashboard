...
const Body = z.object({
  instruments: z.array(z.object({
    ...
    type: z.enum(['EQUITY', 'OPTION', 'INDEX', 'UNDERLYING_SECURITY_FOR_INDEX_OPTION']).default('EQUITY')
  })).min(1).max(250)
});
...
export async function POST(req: Request) {
  ...
  if (env.mock) {
    return NextResponse.json(mock.quotes(parsed.data.instruments));
  }

  const res = await publicFetch(`/userapigateway/marketdata/${encodeURIComponent(accountId)}/quotes`, {
    method: 'POST',
